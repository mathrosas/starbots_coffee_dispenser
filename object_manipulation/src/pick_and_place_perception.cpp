...
public:
  explicit PickAndPlacePerception(const rclcpp::Node::SharedPtr &base_node)
    ...
    sub_pose_ =
        base_node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            ...
            std::bind(&PickAndPlacePerception::poseCb, this,
                      std::placeholders::_1));
    ...
    move_group_node_ =
        rclcpp::Node::make_shared("pick_and_place_perception_node", opts);
    ...
    RCLCPP_INFO(LOGGER, "Ready. Waiting for /cup_pose…");
  }

  void runOnce() {
    // Wait for a transformed /cup_pose (no timeout here)
    rclcpp::Rate r(20);
...
private:
  ...
  void poseCb(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    try {
      cup_pose_base_ = tf_buffer_->transform(*msg, target_frame_);
      have_pose_ = true;
...
int main(int argc, char **argv) {
  ...
  auto base = std::make_shared<rclcpp::Node>("pick_and_place_perception");
  PickAndPlacePerception app(base);
  app.runOnce();
  rclcpp::shutdown();
  return 0;
}
