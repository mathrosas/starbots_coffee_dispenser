...
public:
  ...
  PickPlaceCupBridge() : rclcpp::Node("deliver_cup_bridge") {
    static constexpr const char *kServiceName = "/pick_place_cup";
    static constexpr const char *kActionName = "/deliver_cup";
    ...
    action_client_ = rclcpp_action::create_client<DeliverCup>(
        this, kActionName, action_callback_group_);
    status_pub_ =
    ...
    publish_status("Deliver cup bridge ready. Service: /pick_place_cup");
    RCLCPP_INFO(this->get_logger(),
                "Deliver cup bridge started. Service=/pick_place_cup "
                "Action=/deliver_cup");
  }
  ...
  void handle_service(const std::shared_ptr<PickPlaceCup::Request> request,
...

    {
      ...
      if (service_in_progress_) {
        response->result =
            "Bridge busy: another /pick_place_cup request is in progress";
        publish_status(response->result);
        RCLCPP_WARN(this->get_logger(), "%s", response->result.c_str());
        return;
      }
